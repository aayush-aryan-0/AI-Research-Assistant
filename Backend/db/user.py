...
async def add_user(username:str,full_name:str,email:str,hashed_password:str)->None:
    async with session_local() as session:
        try:
            new_user=__User(username=username,full_name=full_name,email=email,hashed_password=hashed_password)
...
async def update_user(old_username:str,new_username:Optional[str]=None,new_full_name:Optional[str]=None,new_email:Optional[str]=None,new_hashed_password:Optional[str]=None)->None:
    async with session_local() as session:
        try:
            result = await session.execute(select(__User).where(__User.username==old_username))
...
async def get_user(username:str="",email:str="")->__User|None:
    async with session_local() as session:
        try:
            if username: 
                result = await session.execute(select(__User).where(__User.username==username))
                ...
                return user
            if email:
                result = await session.execute(select(__User).where(__User.email==email))
                user=result.scalar_one_or_none()
                if user is None:
                    raise UserNotFoundError()
                return user          
