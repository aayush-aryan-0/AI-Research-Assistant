...
export default function UpdatePasswordForm() {
    ...
    const [newPassword, setNewPassword] = useState<updatePasswordType>({
        new_password: ""
    });
    ...
    async function updateUser() {
        ...
        if (!newPassword.new_password.trim()) {
            setError("Current password is required.");
            return;
        }
      
        const validPassword = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).{8,}$/;
        if (!validPassword.test(newPassword.new_password)) {
            setError("Password must be 8+ characters with uppercase, lowercase, number, and special character.");
            return;
        }
        if (newPassword.new_password !== confirmPassword) {
            setError("Passwords do not match.");
            return;
        }
        
        startTransition(async ()=>{
            const response: APIResponse = await updatePassword(newPassword);
            if (!response.success) {
    ...
    return (
        ...
        <form
            ...
            className="flex flex-col gap-4 rounded-xl w-full max-w-md 
            p-6 bg-white dark:bg-gray-900 border
        ...
        >
            ...
            <div className={fieldClass}>
                ...
                <input id="password" type="password" minLength={8} placeholder="Password"
                    ...
                    onChange={(e)=>setNewPassword({new_password:e.target.value})}
